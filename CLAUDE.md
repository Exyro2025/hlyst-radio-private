...
- **Never trust `beat_track`'s octave, and never re-derive tempo timing from a raw stored BPM.** librosa's 120-centred prior returns the DOUBLE on slow material; `analyze_worker.tempo_octave_fit` settles it from onset strength, and its guards exist specifically so a backbeat catalogue (soul, reggae, funk) isn't halved into nonsense. The beat grid must travel with any correction. → [`docs/internals/music.md`](docs/internals/music.md)

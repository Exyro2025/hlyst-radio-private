...
- **`validateWebhooksStrict` keeps its exact name, signature `(raw: unknown, existing: Webhook[] = [])`, and throwing behaviour.** Its thrown *message text* is allowed to change (zod wording). Its accept/reject decisions **may only become stricter, never looser**, and every divergence from the old validator must be pinned by a test — so a rejection is deliberate rather than accidental. (Amended 2026-08-04 after Task 1 review: the old validator was silently lenient in three spots — `enabled` coerced any non-`false` value to `true`, `authHeader` truncated at 500 chars and defaulted non-strings to `''`, and a malformed `id` was silently re-minted. The schema rejects all three instead. Ruled acceptable: `settings.json` is only ever written by this validator so stored data is always well-formed, and silently truncating a 600-char auth token produces a broken header that fails later at fire time with no explanation. A hand-edited `settings.json` carrying such values will now fail import loudly.)

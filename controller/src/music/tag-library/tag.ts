...
// primary->fallback failover, used in single-LLM mode); `label` is stamped
...
// single-LLM mode); `label` records which configured model owns the batch.
...
// when a fallback is configured, distinct from primary, and answers a cheap
// probe — then both boxes tag in parallel off a shared queue.

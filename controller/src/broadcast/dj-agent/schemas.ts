...
export const PICK_SCHEMA = z.object({
  ...
  reason: z.string().describe('internal scratchpad only — max 12 words, never shown to the listener; do not justify, just note what makes THIS pick a fresh step (new artist, a shift in energy/era/texture), not a vibe label you would recycle pick after pick (e.g. "new artist, lifts the energy", never a repeated "mellow reflective step")'),
    say: z.string().nullable().describe('when the latest event message says to write a spoken link, set this to one or two natural sentences in the DJ voice that INTRODUCE the track you are about to play — set it up, name the artist or capture its feel, vary your opener. Sound like a person reacting in the moment, not a written description: no poetic scene-setting, no metaphor for its own sake, no describing light/rooms/feelings/atmosphere, no phrases like "as we settle into" or "starts to unfurl" — short and spoken, the way you would actually say it, not polished prose. If you do not have a real reaction, keep it minimal or nearly bare. Do NOT back-announce, recap, or name the track that just played (a listener request may slip in ahead of your pick, so what aired right before it is not certain). Never state a clock time unless the event message tells you when the link airs — then use exactly that time. When the event says stay silent, set this to null'),
...
export function pickSchemaBase() {
  ...
  return base.extend({
        say: z.string().nullable().describe(`when the latest event message says to write a spoken link, set this to ${dj.lengthPhrase('link')} of natural speech in the DJ voice that INTRODUCE the track you are about to play — set it up, name the artist or capture its feel, vary your opener. Sound like a person reacting in the moment, not a written description: no poetic scene-setting, no metaphor for its own sake, no describing light/rooms/feelings/atmosphere, no phrases like "as we settle into" or "starts to unfurl" — short and spoken, the way you would actually say it, not polished prose. If you do not have a real reaction, keep it minimal or nearly bare. Do NOT back-announce, recap, or name the track that just played (a listener request may slip in ahead of your pick, so what aired right before it is not certain). ${clockRule} When the event says stay silent, set this to null`),
  });
}
